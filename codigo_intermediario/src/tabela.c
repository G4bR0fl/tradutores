...
params param[100]; // resolver isso com uma matriz
...
void empty_params(params* param){
    int i = 0;
    while(strlen(param[i].argument_type) > 0){
        memset(param[i].argument_type, '\0', strlen(param[i].argument_type));
        i++;
    }
}
